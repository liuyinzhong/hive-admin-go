...
| `POST /api/auth/login` | 公开接口 | 建立登录身份；登录失败记录受防爆破失败计数消费（SYS-ACL-046） |
| `POST /api/public/captcha` | 公开接口 | 签发一次性滑块挑战票据，同 IP 每分钟限 10 次；票据不含用户数据 |
| `/api/auth/profile`（GET 读取、PUT 更新头像和邮箱）、`password`（PUT 修改密码）、`menus`、`codes`、`logout` | 当前用户上下文 | 只处理当前 Token 对应用户，不使用角色行范围；修改密码验证旧密码后只更新本人 |
...
| `/api/dev/changeHistory` | 来源继承 | GET 与 POST 新建按需求、任务、缺陷或版本父对象校验；`PUT /api/dev/changeHistory/{changeId}` 编辑评论时，在父对象范围校验之外另要求当前用户为该评论创建人本人，且仅限 changeBehavior=30 的评论，只读可见性仍完全继承父对象（SYS-DATA-032） |
