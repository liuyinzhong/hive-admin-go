...
当前定义 `0（候诊中）` 和 `30（已完成）`。候诊记录创建时 `queue_status=0`、`call_count=0`。挂号单执行完成接诊时，候诊状态必须在同一事务中由 `0` 更新为 `30`；候诊记录不存在或不处于 `0` 时，挂号单、候诊状态和挂号单日志均不得提交。

叫号次数当前仅预留存储，本期不提供叫号动作。
...
| `queue_status` | 候诊状态，`0` 为候诊中、`30` 为已完成 |
