...
3. 涉及患者、挂号、实际排班或候诊时阅读 [系统数据权限规则](../system/data-permission.md)。
4. 涉及页面时继续阅读前端 [医疗 UI 手册](../../../hive/business-docs/medical/README.md)。
5. 最后核对 Router、Controller、Service、Model、迁移和前端 API。
...
- 临床科室、医生、诊断、挂号费、周期排班模板和排班自动任务是医疗全局主数据或运维记录，由原子接口权限控制，不按创建人切分。
- 患者、挂号单和实际排班按各自 `creator_id` 应用角色数据范围；候诊队列继承实际排班范围。
- 医生工作台、门诊病历和处方按当前排班医生及接诊关系控制；处方审核按审核权限、状态和不得审核本人处方控制。即使角色范围为 `all`，也不能绕过这些领域归属规则。
