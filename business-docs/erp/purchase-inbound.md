...
### ERP-PI-010 入库单按创建人受限并继承来源准入

采购入库单列表和详情按入库单 `creator_id` 应用当前角色数据范围。创建前必须能访问来源采购单，不能只凭采购单 ID 越界收货；累加已有库存余额时还必须能访问该余额。创建成功的入库单、库存流水和新建库存记录归属当前操作者，既有余额不转移归属。打印数据继续继承该入库单范围。
