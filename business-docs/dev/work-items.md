...
需求除手工创建外，可由审批流程通过后的结束后动作落地创建（规则见工作流手册 [runtime.md](../workflow/runtime.md) WF-RUN-050~056）：落地需求固定 storyStatus=0（规划中）、创建人为流程发起人，按表单同名字段映射写入；表单未提供的归属字段留空，无项目归属的需求在编辑补齐项目前不能流转到需要负责人的状态，仅可流转到已关闭。

需求详情的关联流程接口（`GET /dev/storys/{storyNum}/workflow`）返回该需求关联的全部流程实例列表（按绑定时间倒序）：既包含需求创建后自动发起的需求流程，也包含审批流程落地创建该需求时写入的来源审批实例；未关联流程时返回空数组。
