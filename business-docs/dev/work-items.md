...
需求除手工创建外，可由被动触发的需求流程联动：需求创建后按"启动类型=被动触发 + 默认流程标志 + 业务类型=需求 + 已发布"自动匹配并发起流程（管理员在流程定义列表显式设默认，未设默认时需求保持无流程，可手动流转推进状态）；流程节点上挂载的自动化动作（修改字段值）在节点完成时推进需求状态并写入变更记录，规则见工作流手册 [runtime.md](../workflow/runtime.md) WF-RUN-050~054。
