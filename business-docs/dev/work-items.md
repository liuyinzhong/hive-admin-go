...
需求归属项目，可选关联版本和模块，并可关联多名参与用户（通过独立关联表 `dev_story_user` 管理）、文件、类型、级别和来源。支持单条与批量创建。

需求参与人表 `dev_story_user` 每行记录一个用户及其负责的单一需求状态（`story_status` 为 STORY_STATUS 字典值，NULL 表示普通参与人）。同一用户在同一需求中可有多行，对应负责多个状态。参与人维护采用全删全插模式。
...
新建或编辑需求时，参与用户、附件和关联版本必须对操作者可见；新建或编辑任务、缺陷时，执行/处理用户、关联版本和关联需求必须可见。局部修改参与人或执行人同样执行服务端校验，不能通过直接提交 ID 建立越界引用。
