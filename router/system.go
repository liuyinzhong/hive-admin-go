...
func registerSystemRoutes(api *gin.RouterGroup, deps *RouterDeps) {
	...
	system := api.Group("/system", middleware.AuthMiddleware(), dataPermissionMiddleware)
	{
		...
		menus := system.Group("/menus")
		{
			...
			menus.GET("/:id/roles", permissionGuard.Require("system:menu:detail"), systemController.GetMenuGrantedRoles)
			menus.GET("/:id/users", permissionGuard.Require("system:menu:detail"), systemController.GetMenuGrantedUsers)
