...
- 通知中心在顶栏逐条消费菜单消息，与菜单角标同源互通：点击消息项按整组已读口径跳转归属菜单，逐条与全量已读只改变消息状态，消息不可删除。
- 流程引擎在生成待办任务与抄送记录时通过菜单消息和 SSE 分别向办理人与接收人推送提醒；提醒已读与任务处理状态、抄送记录已读相互独立。
- 系统用户的个人签名在流程签署（同意、拒绝、退回）发生时由工作流引擎复制为不可变的签名快照，随流程记录永久保存；审批历史只读快照，不回读个人签名。
- 系统用户授权与医生身份、流程参与者、数据归属等业务校验叠加生效，不能互相替代。用户功能权限按“有效角色授权 ∪ 个人额外授权 − 个人禁止”三元合并生效，个人权限不改变数据范围，数据范围仍完全由角色决定（[ADR 0006](./business-docs/system/adr/0006-user-personal-permission-tri-state.md)）。
